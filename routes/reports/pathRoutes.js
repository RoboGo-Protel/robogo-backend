...
} = require("../../controllers/reports/pathController");
...
router.post('/', async (req, res) => {
  try {
    const saved = await savePathLog(req.body, req.user);
...
router.get('/', async (req, res) => {
  try {
    const logs = await getAllPathLogs(req.user);
    if (logs.length === 0) {
...
router.get('/date/:date', async (req, res) => {
  ...
  const endOfDay = new Date(validDate.setHours(23, 59, 59, 999));
  try {
    const logs = await getPathLogsByDate({
      ...req.user,
      date: validDate,
    });
    if (!logs || logs.length === 0) {
...
router.get('/session/:sessionId', async (req, res) => {
  ...
  try {
    const logs = await getPathLogsBySessionId({
      ...req.user,
      sessionId,
    });
    if (!logs || logs.length === 0) {
...
router.get('/:id', async (req, res) => {
  try {
    const log = await getPathLogById({
      ...req.user,
      id: req.params.id,
    });
    if (!log) {
...
router.delete('/:id', async (req, res) => {
  ...
  try {
    const success = await deletePathLogByID({
      ...req.user,
      id: req.params.id,
    });
    if (!success)
...
router.delete('/date/:date', async (req, res) => {
  ...
  const endOfDay = new Date(validDate.setHours(23, 59, 59, 999));
  try {
    const success = await deletePathLogByDate({
      ...req.user,
      date: validDate,
    });
    if (!success) {
